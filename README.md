...
## The flow of submitting a message
text box submit (component) -> message action creator (action creator) -> the message (view action) ->

chat message util creates the message (util) -> chat web api util recreates the message? then store in localStorage (util) ->

chat server receives the message (action creator) -> the message (server action) ->
